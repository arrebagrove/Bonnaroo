...
namespace Bonnaroo
{
    ...
    public sealed partial class tickets : Page
    {
        ...
        {
            bool exists = await library.checkIfFileExists("ticketLandingPage");
            if (!exists)
            {
                string html = await library.makeWebRequest("http://www.bonnaroo.com/tickets");
                await library.writeFile("ticketLandingPage", html);
                HTMLStrings.Add(new HTMLData(html));
                HtmlSource.Source = HTMLStrings;
            }
            else
            {
                string res = await library.readFile("ticketLandingPage");
